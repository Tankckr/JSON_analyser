...
{
	std::stringstream ss;
	ss << "{\"key2\":123,\"key1\":\"v1\",\"key3\":false}";
	std::shared_ptr<JSON_Object> p = obj_parser.parser(ss)->get_obj();
	p->print(std::cout);
...
TEST(JSON_object, insert)
{
	...
	std::shared_ptr<JSON_Object> p = obj_parser.parser(ss)->get_obj();
	std::shared_ptr<JSON_Value> i = obj_parser.parser(ss);
...
TEST(JSON_array, parser)
{
	...
	std::shared_ptr<JSON_Array> p = arr_parser.parser(ss)->get_arr();
	p->print(std::cout);
	ASSERT_EQ(JARRAY, p->get_type());
	EXPECT_EQ(6, p->get_size());
...
TEST(JSON_string, all)
{
	...
	std::shared_ptr<JSON_String> p = str_parser.parser(ss)->get_str();
...
TEST(JSON_number, all)
{
	...
	std::shared_ptr<JSON_Number> p = num_parser.parser(ss)->get_num();
...
TEST(JSON_bool, all)
{
	std::stringstream ss;
	ss << "truefalse";
	std::shared_ptr<JSON_Bool> p = boo_parser.parser(ss)->get_boo();

	ASSERT_EQ(JBOOL, p->get_type());
	EXPECT_EQ(true, p->get_value());

	p = boo_parser.parser(ss)->get_boo();
	ASSERT_EQ(JBOOL, p->get_type());
...
TEST(JSON_null, all)
{
	std::stringstream ss;
	ss << "null298rw";
	std::shared_ptr<JSON_NULL> p = nul_parser.parser(ss)->get_nul();
